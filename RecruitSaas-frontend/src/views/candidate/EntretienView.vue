<template>
  ...
  <div v-else-if="sessionPhase === 'no_photo'" class="gate-screen">
    <div class="gate-card">
      <div class="gate-photo-icon" aria-hidden="true">
        <svg width="48" height="48" viewBox="0 0 24 24" fill="#1A2B4C"><path d="M20 5h-3.17L15 3H9L7.17 5H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 14H4V7h4.05l1.83-2h4.24l1.83 2H20v12zm-8-1c2.76 0 5-2.24 5-5s-2.24-5-5-5-5 2.24-5 5 2.24 5 5 5zm0-8c1.65 0 3 1.35 3 3s-1.35 3-3 3-3-1.35-3-3 1.35-3 3-3z"/></svg>
      </div>
      <h2 class="gate-h2">Photo de profil requise</h2>
      <p class="gate-sub">Ajoutez une photo à votre profil avant d'accéder à l'entretien. Elle est utilisée pour la vérification biométrique de votre identité.</p>
      <button v-if="!interviewToken" type="button" class="gate-btn" @click="router.push('/profile')">Compléter mon profil</button>
      <p v-else class="gate-hint" style="margin-top:12px">Contactez votre recruteur pour mettre à jour votre profil candidat.</p>
    </div>
  </div>
  ...
  <div v-else class="meet-root">
    <header class="top-bar">
      ...
      <div class="top-center">
        ...
        <!-- Progress questions -->
        <div v-if="interviewMode && interviewQuestions.length" class="question-progress">
          <span class="q-num">Q{{ currentQuestionIndex + 1 }}/{{ interviewQuestions.length }}</span>
          <div class="q-bar"><div class="q-fill" :style="{ width: ((currentQuestionIndex) / interviewQuestions.length * 100) + '%' }"></div></div>
        </div>
      </div>
      <div class="top-right">
      ...
      <transition name="fade">
        <div v-if="interviewMode && currentQ" class="question-card-overlay">
          ...
          <div class="q-voice-status" :class="{ listening: isListeningForAnswer }">
            ...
            {{ isListeningForAnswer ? 'Je vous écoute...' : isSpeaking ? 'Recruteur parle...' : isProcessingAnswer ? 'Traitement en cours...' : 'Appuyez sur le micro pour répondre' }}
          </div>
    ...
    <div v-if="faceCheckPhase === 'running' || faceCheckPhase === 'failed'" class="face-gate-overlay" role="dialog" aria-modal="true" aria-labelledby="face-gate-title">
      <div class="face-gate-card">
        ...
        <!-- Comparaison identité avec la photo de profil -->
        <template v-if="faceCheckPhase === 'running'">
          ...
          <p class="face-gate-sub">Comparaison de votre visage avec la photo de profil sur plusieurs frames…</p>
          <p class="face-gate-hint">Restez face à la caméra, lumière devant vous, sans lunettes de soleil.</p>
        </template>

        <!-- Échec -->
        <template v-else>
          <p id="face-gate-title" class="face-gate-title">Vérification non confirmée</p>
    ...
    <transition name="slide-right">
      <aside class="sidebar" v-if="showSidebar">
        ...
        <div class="session-stats">
          ...
          <div v-if="interviewMode" class="stat-row"><span>Questions</span><span class="stat-val">{{ currentQuestionIndex }}/{{ interviewQuestions.length }}</span></div>
...
const livenessBlinkCount        = ref(0)
...
// ✅ FIX PRINCIPAL : flag de garde contre les appels multiples à handleStructuredAnswer
...
async function verifyInterviewSlot () {
  ...
  if (!t) { interviewPhotoProfilUrl.value = null; return { ok: true, noPhoto: false, data: null } }
  ...
  const photoUrl = data?.photoProfilUrl ?? data?.PhotoProfilUrl ?? null
  interviewPhotoProfilUrl.value = photoUrl
  await loadInterviewQuestions(data)
  return { ok: !!data?.estActif, noPhoto: !photoUrl, data }
}
...
function askCurrentQuestion () {
  ...
  candidateFinal.value = ''
  candidateInterim.value = ''
  showRecruiterBanner(`Question ${currentQuestionIndex.value + 1} / ${interviewQuestions.value.length}`, 'info')
  recruiterSay(q.texte)
}

// ── Gère la réponse structurée ────────────────────────────────────────────────
async function handleStructuredAnswer (reponseText) {
  // ✅ FIX PRINCIPAL : garde contre les appels multiples (double-fire de la reco vocale)
  if (isProcessingAnswer.value) {
    console.debug('[Interview] handleStructuredAnswer ignoré — traitement déjà en cours')
    return
  }
  ...
  // ✅ FIX : stoppe immédiatement la reconnaissance pour éviter tout re-déclenchement
  stopListening()

  console.debug(`[Interview] Traitement réponse Q${currentQuestionIndex.value + 1}:`, reponseText)
  ...
  try {
    ...
    console.warn('[Interview] evaluateAnswer échoué:', e)
  }

  // ✅ FIX : mise à jour immutale correcte du tableau (évite les mutations réactives partielles)
  const updatedQuestions = [...interviewQuestions.value]
  updatedQuestions[currentQuestionIndex.value] = { ...q, reponse: reponseText, score, feedback }
  interviewQuestions.value = updatedQuestions

  if (interviewToken2) {
    saveAnswer(interviewToken2, { questionId: q.id, reponse: reponseText, score, feedback }).catch((e) => {
      console.warn('[Interview] saveAnswer échoué:', e)
    })
  }
  ...
  if (isLast) {
    ...
    const transitions = [
      ...
      "Noté. Passons à la prochaine question."
    ]
    recruiterSay(transitions[currentQuestionIndex.value % transitions.length])
    // ✅ FIX : incrémente l'index AVANT le setTimeout pour qu'il soit cohérent
...
async function finishStructuredInterview () {
  ...
  if (token) {
    ...
    console.warn('[Interview] Token manquant — impossible de finaliser')
    return false
  }
  ...
  if (token) {
    generateReport(
      ...
      if (rapport) {
        return completeEntretien(token, payloadFor(rapport))
      }
    }).catch((e) => {
      console.warn('[Interview] generateReport échoué:', e)
    })
  }
...
async function checkCandidatePhoto () {
  try {
    const res = await api.get('/candidat/profile')
    const url = String(res.data?.avatarUrl ?? res.data?.AvatarUrl ?? '').trim()
    if (!url) { sessionPhase.value = 'no_photo'; return false }
    interviewPhotoProfilUrl.value = url
    return true
  } catch {
    return true
  }
...
async function runInterviewGate () {
  ...
  try {
    const { ok, noPhoto } = await verifyInterviewSlot()
    if (noPhoto) { sessionPhase.value = 'no_photo'; return }
    if (ok) {
    ...
    gatePollTimer = window.setInterval(async () => {
      try {
        const { ok: active, noPhoto: np } = await verifyInterviewSlot()
        if (np) { clearInterval(gatePollTimer); gatePollTimer = null; sessionPhase.value = 'no_photo'; return }
        if (active) {
...
function typeLabel (type) {
  const m = { introduction:'👋 Introduction', technique:'⚙️ Technique', comportementale:'🧠 Comportementale', situation:'📋 Mise en situation', perspective:'🎯 Perspective' }
  return m[type] || type
}
...
// ── Dictionnaire de corrections phonétiques fr-FR → termes techniques ────────
// La Web Speech API en fr-FR translittère les mots anglais/techniques en
// phonétique française. Ce dictionnaire corrige les erreurs les plus courantes.
const TECH_CORRECTIONS = [
  // Langages & frameworks
  [/\bpython\b/gi, 'Python'],
  [/\bjai(?:son)?\b/gi, 'JSON'],         // "jason" → JSON
  [/\bdjango\b/gi, 'Django'],
  [/\bflask\b/gi, 'Flask'],
  [/\bfaste? ?aie?\b/gi, 'FastAPI'],     // "fast aie" → FastAPI
  [/\bvius?\b/gi, 'Vue'],               // "vius" → Vue
  [/\breact\b/gi, 'React'],
  [/\bangulaire?\b/gi, 'Angular'],
  [/\bnode\b/gi, 'Node'],
  [/\bnodé?\b/gi, 'Node'],
  ...
  [/\bdot\s*net\b/gi, '.NET'],
  [/\bpoint\s*net\b/gi, '.NET'],
  [/\bpoint net\b/gi, '.NET'],
  [/\bc\s*di?ès?\s*ze?rp?\b/gi, 'C#'],   // "ci sharp" / "c dièse"
  [/\bci\s*sharpie?\b/gi, 'C#'],
  [/\bc\s*sharp\b/gi, 'C#'],
  [/\bjava\b/gi, 'Java'],
  [/\bphp\b/gi, 'PHP'],
  [/\bruby\b/gi, 'Ruby'],
  ...
  [/\bkotlin\b/gi, 'Kotlin'],
  [/\bswift\b/gi, 'Swift'],
  [/\blaravel\b/gi, 'Laravel'],
  [/\bspring\b/gi, 'Spring'],
  [/\bnext\.?\s*js\b/gi, 'Next.js'],
  [/\bnuxt\.?\s*js\b/gi, 'Nuxt.js'],
  [/\bsvelte\b/gi, 'Svelte'],
  [/\bélastique\s*c\b/gi, 'Elastic'],   // élastique c → Elastic

  // Bases de données
  [/\bsé?qu(?:elle?|ail)\b/gi, 'SQL'],   // "séquèle" / "sequel" → SQL
  [/\bmy\s*sé?qu(?:elle?|ail)\b/gi, 'MySQL'],
  [/\bpost\s*gr[eè]s?\b/gi, 'PostgreSQL'],
  [/\bpostgré?\b/gi, 'PostgreSQL'],
  [/\bmongo\s*(?:db)?\b/gi, 'MongoDB'],
  [/\bredis\b/gi, 'Redis'],
  [/\bcassandra\b/gi, 'Cassandra'],
  [/\belasticsearch\b/gi, 'Elasticsearch'],
  [/\bélastic\s*search\b/gi, 'Elasticsearch'],
  [/\bsqlite\b/gi, 'SQLite'],
  [/\boracle\b/gi, 'Oracle'],
  [/\bdynamo\s*(?:db)?\b/gi, 'DynamoDB'],
  [/\bfire(?:base|baise)\b/gi, 'Firebase'],

  // Cloud & DevOps
  [/\bdocqueur?\b/gi, 'Docker'],         // "dockeur" → Docker
  [/\bdoquer?\b/gi, 'Docker'],
  [/\bkouber(?:nète?s?)?\b/gi, 'Kubernetes'],  // "kouber" → Kubernetes
  [/\bcubernètes?\b/gi, 'Kubernetes'],
  [/\bk8s\b/gi, 'Kubernetes'],
  [/\bterraforme?\b/gi, 'Terraform'],
  [/\bcoubernetis?\b/gi, 'Kubernetes'],
  ...
  [/\bngin?x\b/gi, 'Nginx'],             // "enginx" → Nginx
  [/\benginx\b/gi, 'Nginx'],
  [/\bapache\b/gi, 'Apache'],
  [/\blambda\b/gi, 'Lambda'],
  [/\bkafka\b/gi, 'Kafka'],
  [/\brabb(?:ite?)?mq\b/gi, 'RabbitMQ'],
  [/\blapin\s*(?:mq)?\b/gi, 'RabbitMQ'], // "lapin" → RabbitMQ

  // Architecture & patterns
  [/\brest(?:foul|full?|ful)?\b/gi, 'REST'],
  [/\bgraph\s*(?:ql|quelle?)\b/gi, 'GraphQL'],
  [/\bgraph\s*quelque?\b/gi, 'GraphQL'],
  [/\bgrapes?\s*queue?l?\b/gi, 'GraphQL'],
  ...
  [/\bsoa\b/gi, 'SOA'],
  [/\bcqrs\b/gi, 'CQRS'],
  [/\bévène?ment\s*driven\b/gi, 'event-driven'],
  [/\bserverless\b/gi, 'serverless'],
  [/\bscrum\b/gi, 'Scrum'],
  [/\bagile\b/gi, 'Agile'],
  [/\bkanban\b/gi, 'Kanban'],
  [/\bjira\b/gi, 'Jira'],
  ...
  [/\bskilearn\b/gi, 'scikit-learn'],
  [/\bscikit\b/gi, 'scikit-learn'],
  [/\bllm\b/gi, 'LLM'],
  [/\brag\b/gi, 'RAG'],
  ...
  [/\bpandas\b/gi, 'Pandas'],
  [/\bnumpy?\b/gi, 'NumPy'],
  [/\bmatplotlib\b/gi, 'Matplotlib'],
  [/\bjupyter\b/gi, 'Jupyter'],

  // Concepts généraux IT
  [/\bfronted?\b/gi, 'frontend'],
  [/\bfront\s*end\b/gi, 'frontend'],
  [/\bback\s*end\b/gi, 'backend'],
  [/\bbackend\b/gi, 'backend'],
  [/\bfull\s*stack\b/gi, 'full-stack'],
  [/\bdébug(?:ge?r?)?\b/gi, 'déboguer'],
  [/\bdéploiement\b/gi, 'déploiement'],
  [/\bcloud\b/gi, 'cloud'],
  [/\bscaling?\b/gi, 'scaling'],
  [/\bcaching?\b/gi, 'cache'],
  [/\brefactoring?\b/gi, 'refactoring'],
  [/\brefact(?:o|oriser?)?\b/gi, 'refactoring'],
  [/\bunit\s*tests?\b/gi, 'tests unitaires'],
  [/\btdd\b/gi, 'TDD'],
  [/\bbdd\b/gi, 'BDD'],
  [/\bpoo\b/gi, 'POO'],
  [/\boop\b/gi, 'POO'],
  [/\bsolide?\b/gi, 'SOLID'],              // "solide" → SOLID
  [/\bdesign\s*patterns?\b/gi, 'design patterns'],
  [/\bscrum\s*master\b/gi, 'Scrum Master'],
  [/\bproduct\s*owner\b/gi, 'Product Owner'],
  [/\bpo\b/gi, 'PO'],
  [/\bsprints?\b/gi, 'sprint'],
  [/\bstackoverflow\b/gi, 'Stack Overflow'],
  [/\bstack\s*overflow\b/gi, 'Stack Overflow'],
  [/\blinux\b/gi, 'Linux'],
  [/\bubuntu\b/gi, 'Ubuntu'],
  [/\bwindows\b/gi, 'Windows'],
  [/\bmac\s*os\b/gi, 'macOS'],
  [/\bvs\s*code\b/gi, 'VS Code'],
  [/\bvisual\s*studio\b/gi, 'Visual Studio'],
  [/\bintelij?\b/gi, 'IntelliJ'],
  [/\bjetbrains?\b/gi, 'JetBrains'],
  [/\bnpm\b/gi, 'npm'],
  [/\byarn\b/gi, 'Yarn'],
  [/\bwebpack\b/gi, 'Webpack'],
  [/\bvite\b/gi, 'Vite'],
  [/\blint(?:e?r?)?\b/gi, 'linter'],
  [/\bssl\b/gi, 'SSL'],
  [/\btls\b/gi, 'TLS'],
  [/\bhttps?\b/gi, (m) => m.toUpperCase()],
  [/\bjwt\b/gi, 'JWT'],
  [/\boauth\b/gi, 'OAuth'],
  [/\bsso\b/gi, 'SSO'],
  [/\bldap\b/gi, 'LDAP'],
  ...
  // ── Corrections observées sur transcriptions réelles (fr-FR phonétique) ────

  // Frameworks ML/DL mal phonétisés
  [/\btant?\s*sur\s*fl?ow\b/gi, 'TensorFlow'],      // "tant sur flow" → TensorFlow
  [/\btenseur?\s*fl?ow\b/gi, 'TensorFlow'],
  [/\bpittoresque\b/gi, 'PyTorch'],                  // "pittoresque" → PyTorch
  [/\bpie?\s*torch\b/gi, 'PyTorch'],
  [/\bkéras?\b/gi, 'Keras'],
  [/\bskik?\s*it?\s*learn\b/gi, 'scikit-learn'],
  [/\bxg\s*boost\b/gi, 'XGBoost'],
  [/\bex\s*gé?\s*boost\b/gi, 'XGBoost'],
  [/\blight\s*gbm\b/gi, 'LightGBM'],
  [/\bcat\s*boost\b/gi, 'CatBoost'],
  [/\bmel\s*flow\b/gi, 'MLflow'],                    // "mel flow" → MLflow
  [/\bml\s*flow\b/gi, 'MLflow'],
  [/\bair\s*flow\b/gi, 'Airflow'],
  [/\bkube\s*flow\b/gi, 'Kubeflow'],

  // Architectures réseaux de neurones
  [/\bles\s*RN\b/g, 'les RNN'],                      // "les RN" → les RNN
  [/\bR\s*N\s*N\b/g, 'RNN'],
  [/\bC\s*N\s*N\b/g, 'CNN'],
  [/\bG\s*A\s*N\b/g, 'GAN'],
  [/\bganes?\b/gi, 'GANs'],
  [/\bvé?\s*trans\s*former\b/gi, 'Transformer'],
  [/\btransfo(?:rmer)?\b/gi, 'Transformer'],
  [/\bbert\b/gi, 'BERT'],
  [/\bgé\s*pété?\b/gi, 'GPT'],                       // "gépété" → GPT
  [/\bvé\s*réseaux?\b/gi, 'VAE'],
  [/\bauto\s*encodeur\b/gi, 'auto-encodeur'],
  [/\blstm\b/gi, 'LSTM'],
  [/\bgru\b/gi, 'GRU'],
  [/\battention\s*mécanisme\b/gi, 'mécanisme d\'attention'],

  // Concepts ML phonétisés
  [/\bgrave\s*dyna/gi, 'graphe dyna'],               // "grave dynamique" → graphe dynamique
  [/\brégulière?\s*r?as\s*nation\b/gi, 'régularisation'], // "régulière RAS nation"
  [/\brégularis(?:a|â)tion\b/gi, 'régularisation'],
  [/\bsaut\s*apprent/gi, 'sous-apprent'],            // "saut apprentissage" → sous-apprentissage
  [/\bsur\s*app?rentissage\b/gi, 'sur-apprentissage'],
  [/\bsous\s*app?rentissage\b/gi, 'sous-apprentissage'],
  [/\boverfitting\b/gi, 'overfitting'],
  [/\bunderfitting\b/gi, 'underfitting'],
  [/\bdrop\s*haut\b/gi, 'dropout'],                 // "drop haut" → dropout
  [/\bdropout\b/gi, 'dropout'],
  [/\bbatch\s*norm\b/gi, 'batch normalization'],
  [/\bhyper\s*para?mètres?\b/gi, 'hyperparamètres'],
  [/\bvalidation\s*croisée?\b/gi, 'validation croisée'],
  [/\bcross\s*valid/gi, 'cross-valid'],
  [/\bgrille?\s*search\b/gi, 'grid search'],
  [/\brandom\s*forest\b/gi, 'Random Forest'],
  [/\bforêt\s*aléatoire\b/gi, 'Random Forest'],
  [/\bforê\s*aléa/gi, 'Random Forest'],
  [/\bgradient\s*boost/gi, 'Gradient Boosting'],
  [/\bgradi(?:ent)?\s*des(?:cente?)?\b/gi, 'descente de gradient'],
  [/\bback\s*prop(?:agation)?\b/gi, 'rétropropagation'],
  [/\brétro\s*propagation\b/gi, 'rétropropagation'],
  [/\bembedding\b/gi, 'embedding'],
  [/\bfine\s*tuning\b/gi, 'fine-tuning'],
  [/\bfine?\s*tounin?g?\b/gi, 'fine-tuning'],
  [/\btransfer\s*learn/gi, 'transfer learning'],
  [/\bdata\s*augment/gi, 'data augmentation'],
  [/\baugmentation\s*de\s*données\b/gi, 'augmentation de données'],
  [/\bfeature\s*engineer/gi, 'feature engineering'],
  [/\bfeature\s*extract/gi, 'feature extraction'],
  [/\bfeature\s*select/gi, 'feature selection'],
  [/\bpca\b/gi, 'PCA'],
  [/\bt\s*sné?\b/gi, 't-SNE'],
  [/\bumap\b/gi, 'UMAP'],
  [/\bcluster(?:ing)?\b/gi, 'clustering'],
  [/\bclassifi(?:er|cation)\b/gi, (m) => m],
  [/\brégression\b/gi, 'régression'],
  [/\baccuracy\b/gi, 'accuracy'],
  [/\blose?\s*[àa]\s*currenc\w*/gi, 'loss et accuracy'], // "lose à currency" → loss et accuracy
  [/\bla\s*ten[cs]e?\b/gi, 'latence'],
  [/\bscalab[il]+ité\b/gi, 'scalabilité'],

  // Visualisation mal phonétisée
  [/\bmatelotte?\s*lib\b/gi, 'Matplotlib'],          // "matelotte lib" → Matplotlib
  [/\bmato?\s*plot\s*lib\b/gi, 'Matplotlib'],
  [/\bsi\s*born?\b/gi, 'Seaborn'],                   // "si bonne" → Seaborn
  [/\bsea?\s*born?\b/gi, 'Seaborn'],
  [/\bhits?\s*maps?\b/gi, 'heatmaps'],               // "hits maps" → heatmaps
  [/\bheat\s*map\b/gi, 'heatmap'],
  [/\bplote?\s*ly\b/gi, 'Plotly'],
  [/\bboc\s*é?\b/gi, 'Bokeh'],
  [/\btablo?\b/gi, 'Tableau'],
  [/\bpowerbiai?\b/gi, 'Power BI'],
  [/\bpower\s*bi\b/gi, 'Power BI'],

  // DevOps / MLOps phonétisés
  [/\bconte\s*tenu?eur\s*r?is[eé]r?\b/gi, 'conteneuriser'], // "conte teneur rise" → conteneuriser
  [/\bconte?neur(?:iser|isation)?\b/gi, 'conteneurisation'],
  [/\bfast\s*aie?\b/gi, 'FastAPI'],
  [/\bflask\b/gi, 'Flask'],
  [/\bdjango\s*reuste?\b/gi, 'Django REST'],
  [/\borch?est?ration\b/gi, 'orchestration'],
  [/\bpipe\s*line\b/gi, 'pipeline'],
  [/\bendpoint\b/gi, 'endpoint'],
  [/\bmonitoring\b/gi, 'monitoring'],
  [/\blogging\b/gi, 'logging'],
  [/\bversioning\b/gi, 'versioning'],
  [/\bdéploiement\b/gi, 'déploiement'],

  // Fautes de transcription grammaticales courantes (fr-FR)
  [/\bj['']IA\b/g, "j'ai"],                          // "j'IA" → j'ai (confusion IA/ai)
  [/\banneaux\s*analogies\b/gi, 'analogies'],         // "anneaux analogies" → analogies
  [/\bguide\b/gi, (m, offset, str) => {              // "guide" → Git seulement si contexte versioning
    const ctx = str.slice(Math.max(0, offset - 40), offset + 20).toLowerCase()
    return /version|commit|branch|repo|dépôt/.test(ctx) ? 'Git' : m
  }],
]

function applyTechCorrections (text) {
  let result = text
...
function sanitizeTranscript (t) {
  const cleaned = String(t||'').replace(/\s+/g,' ').replace(/^[\s,.;:!?-]+|[\s,.;:!?-]+$/g,'').trim()
  return applyTechCorrections(cleaned)
}
...
function ensureRecognition () {
  ...
  if (!SR) return null
  const r = new SR()
  // ✅ lang mixte : fr-FR reconnaît mieux les mots français ; maxAlternatives à 3
  // permet de choisir la meilleure alternative si la principale est mal transcrite
  r.lang = 'fr-FR'; r.continuous = true; r.interimResults = true; r.maxAlternatives = 3
  recognition = r
  return r
}
...
function startListening () {
  // ✅ FIX : n'écoute pas si un traitement est en cours
  if (!micOn.value || isSpeaking.value || isProcessingAnswer.value) return
...
function recruiterSay (text, { bannerKind = 'info' } = {}) {
  ...
  speakFrenchOnce(text, avatarRig, {
    ...
    onEnd: () => {
      isListeningForAnswer.value = interviewMode.value && !isProcessingAnswer.value
      recognitionRestartTimer = setTimeout(() => startListening(), 100)
    }
  })
}

// ✅ FIX PRINCIPAL : handleCandidateUtterance vérifie isProcessingAnswer avant tout
function handleCandidateUtterance (text) {
  const clean = sanitizeTranscript(text)
  if (!clean) return

  // ✅ FIX : ignore si un traitement de réponse est déjà en cours
  if (interviewMode.value && isProcessingAnswer.value) {
    console.debug('[Interview] Utterance ignorée — traitement en cours:', clean)
    return
  }
...
// ✅ AMÉLIORATION : score une transcription selon le nombre de termes techniques
// qu'elle contient après corrections — utilisé pour choisir la meilleure alternative
function techScore (text) {
  const corrected = applyTechCorrections(text)
  // un terme technique = mot qui commence par une majuscule ou est un acronyme connu
  const techMatches = corrected.match(/\b([A-Z][a-zA-Z]+|[A-Z]{2,}|\.NET|CI\/CD)\b/g)
  return techMatches ? techMatches.length : 0
}
...
function bindRecognitionHandlers () {
  ...
  r._bound = true
  r.onresult = (event) => {
    // ✅ FIX : ignore les résultats si un traitement est en cours
    if (interviewMode.value && isProcessingAnswer.value) return

    let interim = '', finalText = ''
    for (let i = event.resultIndex; i < event.results.length; i++) {
      const res = event.results[i]
      if (res.isFinal) {
        // ✅ AMÉLIORATION : parmi les maxAlternatives (3), on choisit la transcription
        // qui contient le plus de mots techniques reconnus après corrections.
        let bestTranscript = res[0]?.transcript || ''
        let bestScore = techScore(bestTranscript)
        for (let a = 1; a < res.length; a++) {
          const alt = res[a]?.transcript || ''
          const s = techScore(alt)
          if (s > bestScore) { bestScore = s; bestTranscript = alt }
        }
  ...
  r.onerror = () => {
    recognitionActive = false
    clearRecognitionRestart()
    if (micOn.value && !isSpeaking.value && !isProcessingAnswer.value) {
      recognitionRestartTimer = setTimeout(() => startListening(), 300)
    }
  }
  r.onend = () => {
    recognitionActive = false
    if (micOn.value && !isSpeaking.value && !isProcessingAnswer.value) {
      recognitionRestartTimer = setTimeout(() => startListening(), 150)
    }
  }
}
...
watch(isSpeaking, (v) => {
  if (v) {
    ...
    // ✅ FIX : ne redémarre l'écoute que si aucun traitement n'est en cours
    if (!isProcessingAnswer.value) {
...
// ✅ FIX : watch sur isProcessingAnswer pour relancer l'écoute dès que le traitement est terminé
watch(isProcessingAnswer, (processing) => {
  if (!processing && micOn.value && !isSpeaking.value) {
...
const PHRASES_BIOMETRIC_SKIPPED = [
  ...
  "Présentez-vous brièvement : votre parcours et ce que vous recherchez."
]

async function runFacialIdentityGate () {
  stopListening()
  faceCheckError.value = ''
  ...
  faceBiometricSkipped.value = false

  const mod = await import('@/utils/faceIdentityMatch.js')

  const photoUrl = url.startsWith('http') ? url : `http://localhost:5202${url.startsWith('/') ? '' : '/'}${url}`
  console.debug('[FaceGate] photo URL:', photoUrl)

  faceCheckPhase.value = 'running'
  try {
    const refDescriptor = await mod.descriptorFromPhotoUrl(photoUrl)
    const res = await mod.matchVideoToDescriptor(selfVideo.value, refDescriptor, {
      maxAttempts: 24,
      pauseMs: 400,
      threshold: mod.getMatchThreshold(),
      confirmFrames: 2
    })
    console.debug('[FaceGate] match result:', res)
    if (!res.match) {
      faceCheckPhase.value = 'failed'
      faceCheckError.value = res.distance != null
        ? `Visage non reconnu (écart ${res.distance.toFixed(2)} / seuil ${res.threshold.toFixed(2)}). Éclairez votre visage et restez face caméra.`
        : 'Aucun visage détecté. Vérifiez que la caméra est active et déobstruée.'
      return false
    }
    faceCheckPhase.value = 'ok'
    return true
  } catch (e) {
    console.error('[FaceGate] error:', e)
    faceCheckPhase.value = 'failed'
    faceCheckError.value = e?.message === 'photo_load_failed'
      ? `Impossible de charger la photo de profil (${photoUrl}). Vérifiez que l'API est démarrée.`
      : e?.message === 'no_face_in_profile_photo'
      ? 'Aucun visage trouvé sur la photo de profil. Mettez à jour votre photo de profil.'
      : e?.message || 'Erreur comparaison faciale.'
    return false
  }
}
...
onMounted(async () => {
  if (interviewToken.value) { await runInterviewGate() }
    ...
    const hasPhoto = await checkCandidatePhoto()
    if (!hasPhoto) return
    sessionPhase.value = 'ready'
    await nextTick()
    await bootEntretienSession()
...
.gate-photo-icon{display:flex;justify-content:center;margin-bottom:16px;opacity:0.85}
...
.liveness-eye-anim{display:flex;justify-content:center;margin-bottom:14px}
.liveness-blink-counter{display:flex;align-items:center;gap:10px;margin:14px 0 8px;padding:10px 16px;border-radius:10px;background:#f0fdf4;border:1px solid #bbf7d0}
.blink-dot{width:14px;height:14px;border-radius:50%;background:#d1d5db;transition:background .3s,box-shadow .3s;flex-shrink:0}
.blink-dot.active{background:#10b981;box-shadow:0 0 8px #10b981}
.blink-label{font-size:13px;font-weight:600;color:#374151}
</style>