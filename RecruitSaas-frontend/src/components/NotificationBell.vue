    ...
    <transition name="panel-slide">
      <div v-if="open" class="notif-panel">
        ...
        <div v-else class="notif-list">
          ...
          >
            ...
            <div class="notif-icon" :class="'icon-' + n.type">
              <!-- new_application -->
              <svg v-if="n.type === 'new_application'" width="14" height="14" viewBox="0 0 24 24" fill="none">
              ...
              <!-- eval_reminder -->
              <svg v-else-if="n.type === 'eval_reminder'" width="14" height="14" viewBox="0 0 24 24" fill="none">
        ...
        <div class="panel-foot">
          <button class="view-all-btn" @click="goToSettings">
            Manage preferences
...
const { notifications, unreadCount, initialized, markRead, markAllRead, startPolling, stopPolling } = useNotifications()
...
function handleClick(n) {
  ...
  let role = 'Expert'
  try { role = JSON.parse(atob(token.split('.')[1])).role ?? 'Expert' } catch {}
  
  if (role !== 'Expert') return  // ← pour les autres rôles, juste marquer lu
...
.icon-new_application { background: #eff6ff; color: #2563eb; }
.icon-eval_reminder   { background: #fefce8; color: #ca8a04; }
.icon-status_updates  { background: #f0fdf4; color: #16a34a; }
